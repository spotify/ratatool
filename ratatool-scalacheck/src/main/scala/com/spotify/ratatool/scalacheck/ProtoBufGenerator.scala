...
/**
 * Contains mappings to CodedOutputStream writer functions. Allows creation of all Generators
 * before deriving each generated `Array[Byte]``
 */
private object ProtoBufWriters {
  ...
  case class EnumWriter(id: Int, v: EnumValueDescriptor)(cos: CodedOutputStream)
    extends Writer(id, v)(cos)
  { override def write(): Unit = cos.writeEnum(id, v.getNumber) }

  case class BytesWriter(id: Int, v: Array[Byte])(cos: CodedOutputStream) extends Writer(id, v)(cos)
...
trait ProtoBufGeneratorOps {
  ...
  private def generate(desc: Descriptors.Descriptor): Gen[Array[Byte]] = {
    Gen
      .sequence[List[PartialWriter], PartialWriter](desc.getFields.asScala.map(genField)).map { l =>
  ...
  private def genField(field: FieldDescriptor): Gen[PartialWriter] = {
    ...
    def genV: Gen[PartialWriter] = field.getType match {
      ...
      case Type.BYTES => Gen.choose(1, 40).flatMap(Gen.listOfN(_, Arbitrary.arbByte.arbitrary))
          .map(l => BytesWriter(id, l.toArray))

      case Type.MESSAGE => generate(field.getMessageType)
    ...
    field.toProto.getLabel match {
      case Label.LABEL_OPTIONAL => Arbitrary.arbBool.arbitrary.flatMap { e =>
