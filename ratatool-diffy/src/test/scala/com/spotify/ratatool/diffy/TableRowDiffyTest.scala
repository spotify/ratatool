...
class TableRowDiffyTest extends AnyFlatSpec with Matchers {
  ...
  it should "support nested fields" in {
    ...
    d(x, z2) should equal(Seq(Delta("field1", Option(x.get("field1")), None, UnknownDelta)))
  ...
  it should "support unordered" in {
    ...
    val d = new TableRowDiffy(schema)
    d(x, z) should equal(
      Seq(Delta("field1", Option(jl(a, b, c)), Option(jl(a, c, b)), UnknownDelta))
    )
  }
